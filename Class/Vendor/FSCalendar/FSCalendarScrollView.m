...
// pointsArray set方法
- (void)setPointsArrayBlue:(NSMutableArray<NSString *> *)pointsArray {
    
    FSCalendarDateModel *dateModel = self.monthArray[1];
    dateModel.pointsArrayBlue = [NSMutableArray arrayWithArray:pointsArray];
}
// pointsArrayRed set方法
- (void)setPointsArrayRed:(NSMutableArray<NSString *> *)pointsArrayRed {
    
    FSCalendarDateModel *dateModel = self.monthArray[1];
    dateModel.pointsArrayRed = [NSMutableArray arrayWithArray:pointsArrayRed];
}


// pointsArrayBlue get方法
- (NSMutableArray<NSString *> *)pointsArrayBlue {
    
    FSCalendarDateModel *dateModel = self.monthArray[1];
    return dateModel.pointsArrayBlue;
}
// pointsArrayRed get方法
- (NSMutableArray<NSString *> *)pointsArrayRed {
    
    FSCalendarDateModel *dateModel = self.monthArray[1];
    return dateModel.pointsArrayRed;
}
...
- (void)layoutCollectionViewsDataWithCell:(FSCalendarCell *)cell IndexPath:(NSIndexPath *)indexPath withDataIndex:(NSInteger)index {
    ...
    if (indexPath.row < firstWeekday) {// 上月末尾的几天
        ...
        if (index == 1) {
            ...
            BOOL isHavingRed = NO;// pointsArray 中是否包含当前日期
                       for (NSString *pointString in self.pointsArrayRed) {
                       ...
                       if (isHavingRed) {
                           cell.solarDateLabel.textColor = Color_Text_CurrentMonth_Selected;
                           cell.lunarDateLabel.textColor = Color_Text_CurrentMonth_Selected;
                           cell.currentSelectView.backgroundColor = HEXCOLOR(0xBD1F1F);
                       }
            
            
            BOOL isHaving = NO;// pointsArray 中是否包含当前日期
            for (NSString *pointString in self.pointsArrayBlue) {
...
//            cell.pointView.hidden = !isHaving;
            ...
            if (isHaving) {
                cell.solarDateLabel.textColor = Color_Text_CurrentMonth_Selected;
                cell.lunarDateLabel.textColor = Color_Text_CurrentMonth_Selected;
                cell.currentSelectView.backgroundColor = HEXCOLOR(0x1C81D2);
            }
...
- (void)pushToPreviousMonthOrNextMonthWithPageIndex:(NSInteger)pageIndex {
    ...
    if (!self.ispushToPreviousMonthOrNext) {
        return;
    }
