...
#define CFCA_EXPORT __attribute__ ((visibility ("default")))
...
typedef enum algType {
    CFCA_SYMMETRIC_ALGORITHM_3DES_CBC = 0,
    CFCA_SYMMETRIC_ALGORITHM_RC4,
    CFCA_SYMMETRIC_ALGORITHM_SM4
}CFCAAlgType;

typedef enum hashType {
    CFCA_HASH_ALGORITHM_SHA1 = 0,
    CFCA_HASH_ALGORITHM_SHA256,
    CFCA_HASH_ALGORITHM_SM3,
    CFCA_HASH_ALGORITHM_MD5_SHA1,
    CFCA_HASH_ALGORITHM_SHA384,
    CFCA_HASH_ALGORITHM_SHA512,
}CFCAHashType;
...
@property (readonly) NSString       *subjectDNForRA;//DN for RA (no space inside)
...
@property (readonly) NSData         *publicKey;//certificate public key
...
/*!
@abstract   fetch certificate by given serial number
@parameter  [NSString *] serial number
@result     [SCAPCertificate *] certificate, if not found return nil
*/
- (SCAPCertificate *)getCertificateWithSn: (NSString *)serialNumber;
...
/*!
 ...
 @parameter  [CFCAAlgType] encrypt algorithm   3DES:0  RC_4:1
...
- (NSInteger)envelopeEncrypt:(NSData *)dataToEncrypt
                ...
                symmetricAlg:(CFCAAlgType)algorithm
...
/*!
 @abstract   implement the ecdhe algorithm of SM2
 @parameter  [SCAPCertificate *] certificate which be used to encrypt
 @parameter  [NSString *] PIN value
 @parameter  [NSData *]ra: cient random, 32 bytes.
 @parameter  [NSData *]Pb: server public key, 64 bytes.
 @parameter  [NSData *]Rb: server random public key, 64 bytes.
 @parameter  [NSInteger]klen: length of the session key to be generated.
 @parameter  [NSData **]k: session key generated by sm2 dh algorithm
 @result    [NSInteger] error value, when operation failed
 */
- (NSInteger)sm2dh:(SCAPCertificate *)certificate
               pin:(NSString *)pin
                ra:(NSData *)ra
                Pb:(NSData *)Pb
                Rb:(NSData *)Rb
