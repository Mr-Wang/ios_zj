//
//  WY_PerfectInfo2ViewController.m
...
@interface WY_PerfectInfo2ViewController ()
{
    int lastY;
    NSString *picType;
}
...
@implementation WY_PerfectInfo2ViewController
...
- (void) initMajorViews {
    ...
    NSMutableArray *tempMajorsArr = [NSMutableArray new];
    i = 0;
     for (WY_MajorPhotoModel *mpModel in self.arrMajors) {
        if ([mpModel.professionState isEqualToString:@"1"]) {
            //            continue;
            //遍历新专业 -
            //            BOOL isHaveNew = NO;
            WY_MajorPhotoModel *mpNewModel = nil;
            for (WY_MajorPhotoModel *mpNewModel1 in self.arrMajors) {
                if (![mpNewModel1.professionState isEqualToString:@"1"]) {
                    if ([mpModel.professionCode isEqualToString:mpNewModel1.oldProfessionCode]) {
                        mpNewModel = mpNewModel1;
                        break;
                    }
                }
            }
            if (mpNewModel == nil) {
                mpNewModel = [WY_MajorPhotoModel new];
                mpNewModel.oldProfessionCode = mpModel.professionCode;
                mpNewModel.professionState = @"0";
//                [self.arrMajors addObject:mpNewModel];
                [tempMajorsArr addObject:mpNewModel];
            }else {
                NSLog(@"是编辑 已经有了 无需再次添加");
            }
            ...
            mpNewModel.currentIndex = i +1;
            selfMView.oldModel = mpModel;
            selfMView.sCode = [professionCodes componentsJoinedByString:@","];
            [selfMView showCellByModel:mpNewModel];
    ...
    if (tempMajorsArr.count > 0) {
        [self.arrMajors addObjectsFromArray:tempMajorsArr];
    }
...
- (void)addMajorAction {
    ...
    if (majorsCount >= 5) {
        [SVProgressHUD showErrorWithStatus:@"最多增加5个专业"];
        return;
    }
...
- (void)submitInfo {
    ...
    if (majorsCount <= 0) {
        [SVProgressHUD showErrorWithStatus:@"请至少选择一个专业"];
        return;
    }
    int i = 0;
    for (WY_MajorPhotoModel *majModel in self.arrMajors) {
        if (![majModel.professionState isEqualToString:@"1"]) {
            if (majModel.professionCodeThird.length <=0) {
    ...
    for (WY_MajorPhotoModel *majModel in self.arrMajors) {
        if (![majModel.professionState isEqualToString:@"1"]) {
            ...
            postModel.isOldOrNew = majModel.professionState;
