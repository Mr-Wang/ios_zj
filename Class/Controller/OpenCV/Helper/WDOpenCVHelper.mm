...
+ (UIImage *)UIImageBgTransparent:(UIImage *)withImage {
    int mark = 100;
//    cv::Mat cvMat = [WDOpenCVHelper cvMatFromUIImage:withImage];
    cv::Mat cvMat;
    UIImageToMat(withImage, cvMat);
    ...
    if ( cvMat.channels() == 1 ) {
        ...
        cv::cvtColor( cvMat, grayMat, CV_BGR2BGRA );
    }
     
    for (int x = 0; x < grayMat.rows; x++) {
        for (int y = 0; y < grayMat.cols; y++) {
            cv::Vec4b pixel =  grayMat.at<cv::Vec4b>(x, y);
            if (pixel[0] > mark && pixel[1] > mark & pixel[2] > mark) {
                pixel[0] = 0;
                pixel[1] = 0;
                pixel[2] = 0;
                pixel[3] = 0;
                grayMat.at<cv::Vec4b>(x, y) = pixel;
            }
        }
    }
    
//    return [WDOpenCVHelper UIImageFromCVMat:grayMat];
    UIImage *ret = MatToUIImage(grayMat);
    return ret;
}
