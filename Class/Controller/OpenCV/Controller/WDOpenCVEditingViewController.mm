...
- (void)viewDidLoad {
    ...
    [cancleButton setTitle:@"完成截取" forState:UIControlStateNormal];
    [cancleButton addTarget:self action:@selector(done) forControlEvents:UIControlEventTouchUpInside];
    ...
    imageFrame.origin.y = kWDOpenCVEditingImageMargin ;
    imageFrame.size.width = CGRectGetWidth(self.view.frame) - kWDOpenCVEditingImageMargin * 2;
    imageFrame.size.height = CGRectGetHeight(self.view.frame) - imageFrame.origin.y - kWDOpenCVEditingImageMargin - JCNew64 - JC_TabbarSafeBottomMargin;
        ...
        [WDOpenCVHelper asyncDetectQuadCornersWithImage:self.originImage targetSize:cropFrame.size completionHandler:^(NSDictionary<NSNumber *,NSValue *> * _Nullable quadPoints) {
            if (quadPoints.count == 4) {
                [quadPoints enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull key, NSValue * _Nonnull obj, BOOL * _Nonnull stop) {
                    [self.cropFrameView updatePointValue:obj.CGPointValue cornerType:(WDOpenCVCornerType)key.integerValue];
                }];
            }
        }];
    }
