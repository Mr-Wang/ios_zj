//
//  WY_SettlementRecordViewController.m
...
@interface WY_SettlementRecordViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation WY_SettlementRecordViewController
...
- (void)dataSourceIndex{
    ...
    [[MS_BasicDataController sharedInstance] postWithReturnCode:zj_getbidEvaluationFeeRecord_HTTP params:postDic jsonData:nil showProgressView:NO success:^(id res, NSString *code) {
        if ([code integerValue] == 0 ) {
            if (((NSArray *)res[@"data"]).count > 0) {
                self.arrDataSource = res[@"data"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WY_SettlementRecordTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"WY_SettlementRecordTableViewCell"];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    WY_SettlementRecordTableViewCell *cell = [self tableView:tableView cellForRowAtIndexPath:indexPath];
    NSDictionary *tempModel = self.arrDataSource[indexPath.row];
    [cell showCellByItem:tempModel];
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        [_tableView registerClass:[WY_SettlementRecordTableViewCell class] forCellReuseIdentifier:@"WY_SettlementRecordTableViewCell"];
