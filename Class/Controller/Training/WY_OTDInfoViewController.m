...
@interface WY_OTDInfoViewController ()<WKUIDelegate,WKScriptMessageHandler,WKScriptMessageHandler,WKNavigationDelegate>{
...
@implementation WY_OTDInfoViewController
...
- (void)makeUI {
    ...
    self.webView = [[ZBWKWebView alloc] initWithFrame:CGRectMake(0, 0, MSScreenW, MSScreenH - k360Width(244)-JCNew64 - JC_TabbarSafeBottomMargin)
    ...
    self.emptyView = [[EmptyView alloc]initWithFrame:CGRectMake(0, 0, MSScreenW, MSScreenH-JCNew64 - k360Width(244) - JC_TabbarSafeBottomMargin)];
    ...
    btnLeft = [[UIButton alloc] initWithFrame:CGRectMake(k360Width(15), self.emptyView.bottom - k360Width(50), kScreenWidth - k360Width(30), k360Width(40))];
...
- (void)bindView {
    ...
    [self makeHeaderView];
    //
    self.webView.height = kScreenHeight - JCNew64 - k360Width(244 + 50) - JC_TabbarSafeBottomMargin;
    ...
    if ([[NSDate date] minutesBeforeDate:[NSDate dateWithString:self.mWY_TraCourseDetailModel.LiveEndTime format:@"yyyy-MM-dd HH:mm:ss" timeZone:nil locale:[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"]]] >= 0) {
        ...
        self.webView.height = kScreenHeight - JCNew64 - k360Width(244) - JC_TabbarSafeBottomMargin;
        [btnLeft setHidden:YES];
    }
}

-(void)makeHeaderView{
    self.webHeaderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width,100)];
    
    _webView.isSameColorWithHeaderView = NO;
    _webView.headerView = self.webHeaderView;
    
    UILabel *lblPrice = [[UILabel alloc] initWithFrame:CGRectMake(k360Width(16),  k360Width(16), k360Width(150), k360Width(36))];
    ...
    lblPrice.text = [NSString stringWithFormat:@"￥%@",self.mWY_TraCourseDetailModel.Price];
...
- (void)btnLeftAction {
    ...
    if (self.mUser.idcardnum.length <= 0) {
        [SVProgressHUD showErrorWithStatus:@"请到个人信息页面填写身份证号和姓名"];
