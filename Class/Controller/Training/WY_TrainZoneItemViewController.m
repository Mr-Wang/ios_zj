//
//  WY_TrainZoneItemViewController.m
...
#import "WY_TrainZoneItemViewController.h"
...
@interface WY_TrainZoneItemViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation WY_TrainZoneItemViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"TrainingSearchNotify" object:nil];

    [notifyHomeSearch addObserver:self selector:@selector(trainSearchNotify:) name:@"TrainingSearchNotify" object:nil];
...
- (void)dataSourceIndex{
    ...
    [[MS_BasicDataController sharedInstance] postWithReturnCode:getTraCourseListFromHome_HTTP params:postDic jsonData:nil showProgressView:NO success:^(id res, NSString *code) {
        if ([code integerValue] == 1 ) {
            if (((NSArray *)res[@"data"][@"data"]).count > 0) {
                self.arrDataSource = [NSArray yy_modelArrayWithClass:[WY_TrainItemModel class] json:res[@"data"][@"data"]];
...
- (void)dataSourceNextPage {
    ...
    [[MS_BasicDataController sharedInstance] postWithReturnCode:getTraCourseListFromHome_HTTP params:postDic jsonData:nil showProgressView:NO success:^(id res, NSString *code) {
        if ([code integerValue] == 1 ) {
            NSLog(@"获取数据成功");
            if (((NSArray *)res[@"data"][@"data"]).count > 0) {
                NSArray *tempArr = [NSArray yy_modelArrayWithClass:[WY_TrainItemModel class] json:res[@"data"][@"data"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WY_TrainItemModel *tempModel = self.arrDataSource[indexPath.row];
