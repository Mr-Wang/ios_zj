...
@interface WY_MyTQMainViewController ()
...
@implementation WY_MyTQMainViewController
...
- (void)makeUI {
    ...
    if ([self.mUser.UserType isEqualToString:@"2"]) {
        //企业主
        self.title = @"管理题目";
        self.titleArr =@[@"企业题目",@"待企业主审核",@"企业主审核通过",@"系统审核通过",@"企业主审核拒绝",@"系统审核拒绝"];
    } else {
        //个人
        self.title = @"我的题目";
        self.titleArr =@[@"我的题目",@"待企业主审核",@"企业主审核通过",@"系统审核通过",@"企业主审核拒绝",@"系统审核拒绝"];
...
//    return nil;
    self.selZJIndex = index;
     WY_MyTQListViewController<ZJScrollPageViewChildVcDelegate> *fourview = (WY_MyTQListViewController *)reuseViewController;
     if (!fourview) {
         fourview = [[WY_MyTQListViewController alloc] init];
      }
