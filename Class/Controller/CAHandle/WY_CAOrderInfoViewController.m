//
//  WY_CAOrderInfoViewController.m
...
@interface WY_CAOrderInfoViewController ()
{
}
...
@implementation WY_CAOrderInfoViewController
@synthesize viewSel3Yj,viewSel4Lq,viewSel6Fp,viewSel6NotFp;
...
- (void)initTopView {
    ...
    [img3 setImage:[UIImage imageNamed:@"0611_ws3"]];
    ...
    [lblLine2 setTextColor:HEXCOLOR(0x0F6DD2)];
...
- (void)initMiddleView {
    ...
    [imgukey setFrame:CGRectMake(viewSel1.width - k360Width(75+16), self.lblT3.top, k360Width(75), k360Width(35))];
    ...
    [lblGLPTB setText:self.dicPostCAInfo[@"glpt"]];
    ...
    [self.lblT4 setFrame:CGRectMake(k375Width(16), lblGLPTB.bottom, kScreenWidth - k375Width(32), k375Width(44))];
    ...
    [self.lblT3 setText:self.dicPostCAInfo[@"sqjg"]];
    NSString *yxqStr = @"";
    if ([self.dicPostCAInfo[@"dqsj"] isEqual:[NSNull null]]) {
        yxqStr = @"有 效 期 ：自数字证书(CA)制作之日起一年内有效";
    } else {
        yxqStr = [NSString stringWithFormat:@"有效期至：%@",self.dicPostCAInfo[@"dqsj"]];
    }
    NSMutableAttributedString *moneyStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"订单编号：%@\n金      额：%.2f元\n%@\n订单日期：%@\n订单状态：",self.dicPostCAInfo[@"OrderNo"],[self.dicPostCAInfo[@"amountPrice"] floatValue],yxqStr,self.dicPostCAInfo[@"createDate"]]];
    ...
    [btnEdit setTitle:@"修改订单" forState:UIControlStateNormal];
    ...
    } else if ([self.dicPostCAInfo[@"status"] intValue] == 1 && ([self.dicPostCAInfo[@"orderStatus"] intValue] == 3 || [self.dicPostCAInfo[@"orderStatus"] intValue] == 4) && ([self.dicPostCAInfo[@"handleType"] intValue] == 1)) {
//    } else if (1==1) {
        //如果审核状态是已通过、 并且发货状态是已发货或已收货、并且是新版订单
        [btnEdit setHidden:NO];
        [btnEdit setTitle:@"CA补办" forState:UIControlStateNormal];
        
        [btnEdit addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
            self.mUser = [[MS_BasicDataController sharedInstance].user yy_modelCopy];
            
            NSMutableDictionary *postDic = [NSMutableDictionary new];
            [postDic setObject:self.dicPostCAInfo[@"idcardnum"] forKey:@"idcardnum"];
            [postDic setObject:self.mUser.UserGuid forKey:@"userGuid"];
            
            [[MS_BasicDataController sharedInstance] postWithReturnCode:zj_getBBStatus_HTTP params:postDic jsonData:nil showProgressView:YES success:^(id res, NSString *code) {
                if (([code integerValue] == 0)) {
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"申请补办后，原数字证书（CA）将不能继续使用，请确认是否补办。" preferredStyle:UIAlertControllerStyleAlert];
                    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                        NSLog(@"确定");
                        WY_CAMakeUpViewController *tempController = [WY_CAMakeUpViewController new];
                        tempController.isEdit = @"0";
                        tempController.dicEditInfo = self.dicPostCAInfo;
                        [self.navigationController pushViewController:tempController animated:YES];
                    }]];
                    [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
        ...
        self.btnYanQi = [UIButton new];
        [self.btnYanQi setFrame:CGRectMake(btnEdit.left - k360Width(10) - btnEdit.width, btnEdit.top, btnEdit.width, btnEdit.height)];
        [self.btnYanQi rounded:k360Width(30/4)];
        [self.btnYanQi setBackgroundColor:MSTHEMEColor];
        [self.btnYanQi.titleLabel setFont:WY_FONTRegular(12)];
        [self.btnYanQi setTitle:@"CA延期" forState:UIControlStateNormal];
        [viewSel1 addSubview:self.btnYanQi];
        [self.btnYanQi addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
            NSLog(@"点击了CA延期");
            MS_WKwebviewsViewController *wk = [[MS_WKwebviewsViewController alloc] init];
            wk.titleStr = @"CA延期说明";
            wk.webviewURL = @"https://www.capass.cn/Avatar/sqcaczsm.pdf";
    ...
    //判断是否显示按钮
    [self.btnRePay setFrame:CGRectMake(k360Width(264), self.lblT4.bottom + k360Width(5), k360Width(85), k360Width(30))];
    ...
    //判断是否显示按钮
    [self.btnDel setFrame:CGRectMake(k360Width(264-95 - 95), self.lblT4.bottom + k360Width(5), k360Width(85), k360Width(30))];
...
# warning 注释打开可用
    if ([self.dicPostCAInfo[@"orderStatus"] intValue] == 1) {
        ...
        [self.btnEdit setTitle:@"修改订单" forState:UIControlStateNormal];
        [self.btnEdit setHidden:NO];
        self.btnRePay.left = self.btnDel.right + k360Width(10);
        self.btnEdit.left = self.btnRePay.right + k360Width(10);
        ...
        }  else if ([self.dicPostCAInfo[@"status"] intValue] == 1 && ([self.dicPostCAInfo[@"orderStatus"] intValue] == 3 || [self.dicPostCAInfo[@"orderStatus"] intValue] == 4)) {
            viewSel1.height = btnEdit.bottom + k360Width(10);
        }else {
            viewSel1.height = self.lblT4.bottom + k360Width(10);
        }
...
# warning 注释打开可用
    ...
    if  (self.dicPostCAInfo[@"fpUrl"] != nil && ![self.dicPostCAInfo[@"fpUrl"] isEqual:[NSNull null]] && ![self.dicPostCAInfo[@"fpUrl"] isEqualToString:@""]) {
        fpztStr = @"已开票";
        [imgDzfq setHidden:NO];
    }
    NSMutableAttributedString *attFpStr1 = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"发票抬头：%@\n发票金额：%.2f元\n开票状态：%@",self.dicPostCAInfo[@"sqrxm"],[self.dicPostCAInfo[@"amountPrice"] floatValue],fpztStr]];
    
    NSMutableAttributedString *attFpStr2 = [[NSMutableAttributedString alloc] initWithString:@"\n*数字证书(CA)领取后24小时自动开具发票，开具后在此处下载"];
...
# warning 注释打开可用
    ...
    if  (self.dicPostCAInfo[@"fpUrl"] != nil && ![self.dicPostCAInfo[@"fpUrl"] isEqual:[NSNull null]] && ![self.dicPostCAInfo[@"fpUrl"] isEqualToString:@""]) {
        ...
        [btnFpShow addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
            ...
            if (@available(iOS 10.0, *)) {
                [[UIApplication sharedApplication] openURL:[NSURL URLWithString:self.dicPostCAInfo[@"fpUrl"]] options:@{} completionHandler:nil];
            } else {
                [[UIApplication sharedApplication] openURL: [NSURL URLWithString:self.dicPostCAInfo[@"fpUrl"]]];
            }
        }];
    ...
    if ([self.dicPostCAInfo[@"isInvoice"] floatValue] == 1) {
...
- (void)weiXinPay {
    ...
    tempPayModel.body = self.dicPostCAInfo[@"sqjg"];
