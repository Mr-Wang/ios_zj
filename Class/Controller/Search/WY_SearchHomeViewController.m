//
//  WY_SearchHomeViewController.m
//  DormitoryManagementPro
//
//  Created by 王杨 on 2020/3/18.
...
@interface WY_SearchHomeViewController () <UITextFieldDelegate> {
    UIView *viewTop;
...
@implementation WY_SearchHomeViewController
...
//    return nil;
    ...
    if (index == 0) {
        ...
        WY_VideoZoneItemViewController<ZJScrollPageViewChildVcDelegate> *fourview = (WY_VideoZoneItemViewController *)reuseViewController;
        if (!fourview) {
            fourview = [[WY_VideoZoneItemViewController alloc] init];
        }
        fourview.keyword = self.keyword;
         return fourview;
    }
...
- (void) bindDataSearchView {
    [[MS_BasicDataController sharedInstance] postWithURL:getUserkey_HTTP params:nil jsonData:nil showProgressView:NO success:^(id successCallBack) {
        NSLog(@"获取数据成功");
        if (successCallBack) {
//            self.arrgrList = [NSArray yy_modelArrayWithClass:[WY_ReShousListModel class] json:successCallBack[@"grList"]];

            NSUserDefaults *userDef = [NSUserDefaults standardUserDefaults];
            NSString *dicSearchHomeStr = [userDef objectForKey:@"dicSearchHome"];
             if (dicSearchHomeStr) {
            ...
            self.arrreList = [NSArray yy_modelArrayWithClass:[WY_ReShousListModel class] json:successCallBack[@"reSList"]];
            [self bindSearchView];
        } else {
...
- (void) bindSearchView {
    ...
    [lblTitle2 setFrame:CGRectMake(k360Width(16), lastY + k360Width(10), kScreenWidth - k360Width(32), k360Width(44))];
    lblTitle2.text = @"热门搜索";
    [lblTitle2 setFont:WY_FONTMedium(16)];
    [self.viewSearch addSubview:lblTitle2];
    
    lastY = lblTitle2.bottom;
    
    NSMutableArray * tempArrreList = [NSMutableArray new];
    for (WY_ReShousListModel *modelItem in self.arrreList) {
                modelItem.keyword = [modelItem.keyword stringByTrim];
        if (modelItem.keyword.length > 0) {
            [tempArrreList addObject:modelItem];
        }
    }

    if (tempArrreList.count) {
        float Start_X  = k360Width(16);           // 第一个按钮的X坐标
        float Start_Y = lastY + k360Width(16);       // 第一个按钮的Y坐标
        float  Width_Space = k360Width(10);     // 2个按钮之间的横间距
        float  Height_Space = k360Width(8);// 竖间距
        ...
        int maxCount = tempArrreList.count;
        if (maxCount > 9) {
            maxCount = 9;
        }
            for (int i =0; i < maxCount; i ++) {
            ...
            // 圆角按钮
            UIButton *aBt = [[UIButton alloc]initWithFrame:CGRectMake(index * (Button_Width + Width_Space) + Start_X, page  * (Button_Height + Height_Space)+Start_Y, Button_Width, Button_Height)];
                WY_ReShousListModel *modelItem = tempArrreList[i];
                if (modelItem.keyword) {
                    [aBt setTitle:modelItem.keyword forState:UIControlStateNormal];
                } else {
                    [aBt setTitle:@"" forState:UIControlStateNormal];
                }
                [aBt setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                [aBt rounded:k360Width(44/8) width:1 color:APPTextGayColor];
                [aBt addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
                    [self seetDicSearchHome:modelItem.keyword];
                    [self bindView:modelItem.keyword];
                }];
                [self.viewSearch addSubview:aBt];
                lastY = aBt.bottom;
         }
...
- (void)seetDicSearchHome:(NSString *)txtStr {
    ...
    if (txtStr.length > 0) {
        ...
        [userDef setObject:[arrSearchList componentsJoinedByString:@"rf!g"] forKey:@"dicSearchHome"];
    }
}
...
- (void)bindView:(NSString *)searchText {
    ...
    [notifyHomeSearch postNotificationName:@"HomeSearchnNotify" object:searchText];
}
@end
