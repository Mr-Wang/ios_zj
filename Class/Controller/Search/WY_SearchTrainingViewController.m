//
//  WY_SearchTrainingViewController.m
...
@interface WY_SearchTrainingViewController () <UITextFieldDelegate> {
    UIView *viewTop;
...
@implementation WY_SearchTrainingViewController
...
//    return nil;
    ...
    WY_TrainZoneItemViewController<ZJScrollPageViewChildVcDelegate> *fourview = (WY_TrainZoneItemViewController *)reuseViewController;
    if (!fourview) {
        fourview = [[WY_TrainZoneItemViewController alloc] init];
    }
...
- (void)initHeaderView {
    ...
    txtSearch = [[UITextField alloc] initWithFrame:CGRectMake(k375Width(30 + 16), MH_APPLICATION_STATUS_BAR_HEIGHT + k360Width(10), k360Width(290), k360Width(30))];
...
- (void) bindDataSearchView {
     ...
     NSString *dicSearchHomeStr = [userDef objectForKey:@"dicSearchTraining"];
      if (dicSearchHomeStr) {
...
- (void)seetDicSearchHome:(NSString *)txtStr {
    ...
    if (txtStr.length > 0) {
        ...
        [userDef setObject:[arrSearchList componentsJoinedByString:@"rf!g"] forKey:@"dicSearchTraining"];
    }
}
...
- (void)bindView:(NSString *)searchText {
    ...
    [notifyHomeSearch postNotificationName:@"TrainingSearchNotify" object:searchText];
}
@end
