//
//  WY_CloudSignatureHandleViewController.m
...
@interface WY_CloudSignatureHandleViewController ()
{
    int lastY;
    NSString *picType;
}
...
@implementation WY_CloudSignatureHandleViewController
    ...
    if (![self.isEdit isEqualToString:@"1"] && ![self.isEdit isEqualToString:@"2"]) {
        ...
        [[MS_BasicDataController sharedInstance] getWithReturnCode:zj_sysGetInfo_HTTP params:nil jsonData:nil showProgressView:YES success:^(id res, NSString *code) {
            if (([code integerValue] == 0 || [code integerValue] == 1) && res) {
                ...
                if (self.mUser.city != nil && ![self.mUser.city isEqual:[NSNull null]]) {
                    self.lblAddress.text = self.mUser.city;
                }
...
//                    [self.btnIdCardBClose setHidden:NO];
        ...
        self.mUser.city = self.dicEditInfo[@"city"];
        self.mUser.cityCode = self.dicEditInfo[@"cityCode"];
...
//    [lblT1 setText:@"办理专家CA数字证书（UKEY）"];
    ...
    NSMutableAttributedString *strBl = [[NSMutableAttributedString alloc] initWithString:@"云签章"];
...
//    [self.mScrollView addSubview:viewSel3];
    ...
    self.lblAddress.text = @"请选择专家所属地区";
...
//    [self anitCellTitle:@"专家所属地区" byUITextField:self.txtzjDanWeiName];
//    [self anitCellTitle:@"通讯地址" byUITextField:self.txtzjDanWeiAddress];
    
    WS(weakSelf)
    [self initCellTitle:@"专家所属地区" byLabel:self.lblAddress isAcc:YES withBlcok:^{
        NSLog(@"选择专家所属地区");
        
        if (weakSelf.mUser.city.length <= 0) {
            if (weakSelf.mUser.cityList) {
                NSMutableArray *cityStrArr = [NSMutableArray new];
                for (WY_CityModel *cityModel in weakSelf.mUser.cityList) {
                    [cityStrArr addObject:cityModel.cityname];
                }
                
                [ActionSheetStringPicker showPickerWithTitle:@"请选择专家所属地区" rows:cityStrArr initialSelection:0 doneBlock:^(ActionSheetStringPicker *picker, NSInteger selectedIndex, id selectedValue) {
                    WY_CityModel *cityModel =  weakSelf.mUser.cityList[selectedIndex];
                    weakSelf.mUser.city = cityModel.cityname;
                    weakSelf.mUser.cityCode = cityModel.residentId;
            ...
            [SVProgressHUD showErrorWithStatus:@"专家所属地区不可修改"];
            return;
        }
    }];
...
- (void)btnSubmitAction {
    ...
    if (self.mUser.city.length <= 0) {
        [SVProgressHUD showErrorWithStatus:@"请选择专家所属地区"];
        return;
    }
    ...
    [[MS_BasicDataController sharedInstance] postWithReturnCode:zj_getCloudSignaturePrice_HTTP params:postDic jsonData:nil showProgressView:YES success:^(id res, NSString *code) {
        if (([code integerValue] == 0) && res) {
            ...
            [dicCAInfo setObject:self.mUser.city forKey:@"city"];
            [dicCAInfo setObject:self.mUser.cityCode forKey:@"cityCode"];
