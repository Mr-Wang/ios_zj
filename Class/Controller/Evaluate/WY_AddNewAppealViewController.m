//
//  WY_AddNewAppealViewController.m
...
@interface WY_AddNewAppealViewController ()
{
    int lastY;
}
...
@implementation WY_AddNewAppealViewController
...
//
//             //申诉文字
//        self.txtContent.text = paramModel.reconsiderText;
//        self.qualificationEleId = paramModel.reconsiderImage;
//        [self.txtContent setUserInteractionEnabled:NO];
//        self.arrZgzs = [[NSMutableArray alloc] initWithArray:[self.qualificationEleId componentsSeparatedByString:@","]];
//        [self initZgzsImgsShow];
//
...
- (void) makeUI {
    ...
    [self.cancleButton setTitle:@" 发布申诉 " forState:UIControlStateNormal];
...
//    [viewPjBg rounded:8 width:1 color:APPLineColor];
    ...
    [self.txtContent setPlaceholder:@"请输入您的申诉原因"];
    [self.txtContent setBackgroundColor:HEXCOLOR(0xfafafa)];
    [self.txtContent setFont:WY_FONTRegular(14)];

    
    [self.scrollViewZgzs setFrame:CGRectMake(k360Width(0), self.txtContent.bottom  + k360Width(16), kScreenWidth - k360Width(0), k375Width(120))];
    self.arrZgzs = [[NSMutableArray alloc] initWithArray:[self.qualificationEleId componentsSeparatedByString:@","]];
...
- (void)rightBtnAction {
    ...
    /*
     bidevaluation/appealExpertEvaluate
     appeal  申诉详情
     appealAppendix 申诉附件
     id 评价详情id
     */
...
//        //评价文字
//    paramModel.appeal = self.txtContent.text;
//    paramModel.appealAppendix  = self.qualificationEleId;
//
    NSMutableDictionary *dicPost = [NSMutableDictionary new];
    [dicPost setObject:self.mWY_ExpertModel.id forKey:@"id"];
    [dicPost setObject:self.txtContent.text forKey:@"appeal"];
    [dicPost setObject:self.qualificationEleId forKey:@"appealAppendix"];
    
    
//     NSLog(@"JSON:%@",[paramModel toJSONString]);
    [[MS_BasicDataController sharedInstance] postWithURL:zj_appealExpertEvaluate_HTTP params:dicPost jsonData:nil showProgressView:YES success:^(id successCallBack) {
        NSLog(@"获取数据成功");
