...
@interface WY_AddAppealViewController ()
{
    int lastY;
}
...
@implementation WY_AddAppealViewController
...
- (void)addZgzsPicAction {
    ...
    if (self.arrZgzs.count >=2) {
        UIAlertView *alertAction = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"最多添加两张申诉图片" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertAction show];
        return;
    }
...
//    代理机构代码
    ...
    paramModel.expertName = self.mUser.realname;
        //评价文字
    paramModel.reconsiderText = self.txtContent.text;
    
    
    paramModel.reconsiderImage  = self.qualificationEleId;
    
    
     NSLog(@"JSON:%@",[paramModel toJSONString]);
    [[MS_BasicDataController sharedInstance] postWithURL:zj_expertReconsider_HTTP params:nil jsonData:[paramModel toJSONData] showProgressView:YES success:^(id successCallBack) {
        NSLog(@"获取数据成功");
