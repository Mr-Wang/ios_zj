//
//  WY_ReadZoneItemViewController.m
...
@interface WY_ReadZoneItemViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation WY_ReadZoneItemViewController
...
- (void)viewDidLoad {
    ...
    [notifyCenter removeObserver:self name:@"updateCategoryNumNotify" object:nil];
    [notifyCenter removeObserver:self name:@"HomeSearchnNotify" object:nil];
    
    [notifyCenter addObserver:self selector:@selector(updateCategoryNumNotify:) name:@"updateCategoryNumNotify" object:nil];
    
    [notifyCenter addObserver:self selector:@selector(homeSearchNotify:) name:@"HomeSearchnNotify" object:nil];
...
- (void)DataSource {
    if ([self.isItemClicked isEqualToString:@"1"]) {
        ...
        if (!self.selCategorynum) {
            switch (self.idx) {
                case 0:
                    self.selCategorynum = @"5001";
                    
                    break;
                case 1:
                    self.selCategorynum = @"5002";
                    
                    break;
                case 2:
                    self.selCategorynum = @"5003";
                    
                    break;
                case 3:
                    self.selCategorynum = @"5004";
                    
                    break;
                case 4:
                    self.selCategorynum = @"5005";
                    
                    break;
                case 66:
                    self.selCategorynum = @"002";
        ...
        if ([self.isTrack isEqualToString:@"1"]) {
            if (self.idx == 0) {
                //             isread  1 已读，2 未读
                self.isRead = @"2";
            } else if (self.idx == 1){
...
- (void) makeUI {
    ...
    self.mWY_FilterTreeMain.selFilterModelBlock = ^(WY_FilterTreeModel * _Nonnull ftModel) {
        if (ftModel != nil) {
            weakSelf.selCategorynum = ftModel.nsID;
        } else {
            weakSelf.selCategorynum = @"5001";
        }
        [weakSelf dataSourceIndex];
...
- (void)dataSourceIndex{
    ...
    if ([self.isTrack isEqualToString:@"1"]) {
        ...
        [postDic setObject:self.isRead forKey:@"isread"];
    } else {
        if (self.keyword) {
