//
//  WY_QRPersonalListViewController.m
...
@interface WY_QRPersonalListViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation WY_QRPersonalListViewController
...
//    if ([self.nsType isEqualToString:@"2"]) {
        self.tableView.height = kScreenHeight - JCNew64 - JC_TabbarSafeBottomMargin;
//    }
//
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.nsType isEqualToString:@"2"]) {
        tempModel.khmcStr = tempModel.title;
        [cell showCellType2ByItem:tempModel ByNum:indexPath.row];
    } else {
        [cell showCellByItem:tempModel ByNum:indexPath.row];
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.nsType isEqualToString:@"2"]) {
        //进入pdf 页；
        WY_PersonalScoreModel *tempModel = self.arrDataSource[indexPath.row];
        NSLog(@"进入pdf：%@",tempModel.score);
        WY_ShowPdfViewController *tempController = [WY_ShowPdfViewController new];
        tempController.titleStr = @"个人培训考核报告";
        tempController.mWY_PersonalScoreModel = tempModel;
