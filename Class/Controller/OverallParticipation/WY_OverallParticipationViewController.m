//
//  WY_OverallParticipationViewController.m
...
@interface WY_OverallParticipationViewController ()
...
@implementation WY_OverallParticipationViewController
...
//    self.lblTopTitle.text = [NSString stringWithFormat:@"总体参评得分：%@分",self.bonusPoints];
...
- (void)dataSourceIndex{
    ...
    //奖励加分列表
    [[MS_BasicDataController sharedInstance] getWithReturnCode:zj_getCallResult_HTTP params:postDic jsonData:nil showProgressView:YES success:^(id res, NSString *code) {
        NSLog(@"获取数据成功");
...
//    tempModel.unanswered = self.dicDataSource[@"unanswered"];
//    tempModel.refuse = self.dicDataSource[@"refuse"];
    
    tempModel.unansweredDeductPoints = self.unansweredDeductPoints;
    tempModel.refuseDeductPoints = self.refuseDeductPoints;
    
    tempModel.refuseList = self.dicDataSource[@"list"][@"refuse"];
    tempModel.unansweredList = self.dicDataSource[@"list"][@"unanswered"];
    tempModel.unDialedList = self.dicDataSource[@"list"][@"unDialed"];
     
    if (indexPath.row == 0) {
        tempModel.ztType = @"1";
        cell.mUser = self.mUser;
       [cell showCellZTCPByItem:tempModel];
    } else {
        tempModel.ztType = @"2";
        cell.mUser = self.mUser;
       [cell showCellZTCPByItem:tempModel];
