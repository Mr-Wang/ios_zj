...
- (void)completeTransaction:(SKPaymentTransaction *)transaction{
  // Your application should implement these two methods.
    ...
    if ([productIdentifier length] > 0) {
        // 向自己的服务器验证购买凭证
        [self verifyMyServicePurchaseWithPaymentTransaction:transaction];
    }
 
//    [self verifyPurchaseWithPaymentTransaction:transaction isTestServer:NO];
}
- (void)verifyMyServicePurchaseWithPaymentTransaction:(SKPaymentTransaction *)transaction {
    //交易验证
...
//    [self handleActionWithType:SIAPPurchSuccess data:receipt];
 ...
 NSData *requestData = [NSJSONSerialization dataWithJSONObject:requestContents
    ...
    [dicPost setValue:[requestContents mj_JSONString] forKey:@"voucher"];
     //云签章
    [dicPost setObject:@"108" forKey:@"amount"];
    [dicPost setObject:self.tempPayModel.orderGuid forKey:@"orderGuid"];
    [dicPost setObject:self.tempPayModel.out_trade_no forKey:@"OrderNo"];
    [dicPost setObject:[MS_BasicDataController sharedInstance].user.UserGuid forKey:@"userGuid"];
    //云签章-苹果支付接口
    [[MS_BasicDataController sharedInstance] postWithReturnCode:zj_payiosPay_HTTP params:nil jsonData:[dicPost mj_JSONData] showProgressView:YES success:^(id res, NSString *code) {
              if ([code integerValue] == 0) {
    //              [SVProgressHUD showSuccessWithStatus:res[@"msg"]];
                  NSString *nstotal = @"0";
                  if (![res[@"data"] isEqual:[NSNull null]]) {
                      if (![res[@"data"][@"total"] isEqual:[NSNull null]]) {
                          nstotal = [NSString stringWithFormat:@"%@",res[@"data"][@"total"]];
                      }
                  }
                  [self handleActionWithType:SIAPPurchVerSuccess data:[nstotal dataValue]];
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response{
    ...
    if([product count] <= 0){
        ...
        NSLog(@"--------------没有商品------------------");
        if(_handle){
            _handle(SIAPPurchNotCommodity,nil);
        }
#endif
        return;
    }
