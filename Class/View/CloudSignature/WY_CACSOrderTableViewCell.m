...
@implementation WY_CACSOrderTableViewCell
...
- (void)makeUI {
    ...
    [self.imgukey setImage:[UIImage imageNamed:@"0420_cloudsign_sel"]];
...
- (void)showCellByItem:(NSMutableDictionary *)withWY_CompleteStatusModel
{
    ...
    [self.lblDate setText:withWY_CompleteStatusModel[@"createTime"]];
    ...
    [self.lblTitle setText:withWY_CompleteStatusModel[@"productDescription"]];
    [self.lblTitle setTextColor:[UIColor blackColor]];
    [self.lblTitle setFont:WY_FONT375Medium(16)];
    [self.imgukey setFrame:CGRectMake(self.viewCont.width - k360Width(75+16), self.lblTitle.top, k360Width(75), k360Width(75))];
    ...
    NSString *yxqStr = @"";
    if (![withWY_CompleteStatusModel[@"signCertificateSerial"] isNotBlank]) {
        yxqStr = [NSString stringWithFormat:@"有 效 期 ：自数字证书(CA)制作之日起%@内有效",withWY_CompleteStatusModel[@"validityPeriod"]];
    } else {
        yxqStr = [NSString stringWithFormat:@"有效期至：%@",withWY_CompleteStatusModel[@"expireDate"]];
    }
    
    NSMutableAttributedString *attC1Str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"金      额：%.2f元\n%@\n订单状态：",[withWY_CompleteStatusModel[@"totalPrice"] floatValue],yxqStr]];
    NSString *statusTitle = @"";
    UIColor *statusColor = HEXCOLOR(0xEA0000);
    //status  0待审核 、1 审核通过、2 审核未通过、
//    orderStatus  1待支付 、2已支付、3 已发货 4 已领取5已取消
