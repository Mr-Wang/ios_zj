 ...
 */
- (void)showOnParent:(UIView *)parentView position:(WProgressPos)pos withFrame:(CGRect)withFrame
{
    self.position = pos;
    self.parentView = parentView;
    CGRect frame = withFrame;//[self decideTargetFrame:parentView];
    self.frame = frame;
...
//    }
    ...
    [colors addObject:(id)HEXCOLOR(0xF7CD05).CGColor];
    [colors addObject:(id)HEXCOLOR(0xF73E05).CGColor];
    ...
    self.mask.backgroundColor = HEXCOLOR(0xD8D8D8).CGColor;
    
//    CAShapeLayer *userMaskLayer = [CAShapeLayer layer];
//    userMaskLayer.bounds = self.bounds;
//    userMaskLayer.path   = [UIBezierPath bezierPathWithArcCenter:CGPointMake(0, 0) radius:50 startAngle:0 endAngle:2*M_PI clockwise:YES].CGPath;
//    userMaskLayer.position  = CGPointMake(50, 50);
//    [self.mask addSublayer:userMaskLayer];
